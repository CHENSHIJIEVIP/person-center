...
(function($, window, document, undefined) {
	...
	var Slide = function(ele, opt) {
		this.$element = ele,
			this.defaults = {
				...
				vOffset: 5,
				vSpace: 5,
				imgName: ['1.jpg', '2.jpg','3.jpg', '4.jpg','5.jpg', '6.jpg','7.jpg', '8.jpg','9.jpg', '10.jpg','11.jpg', '12.jpg','13.jpg', '14.jpg','15.jpg', '16.jpg','17.jpg', '18.jpg','19.jpg', '20.jpg'],
				imgSize: {
					width: '400px',
					height: '200px',
				},
	...
	Slide.prototype = {
		...
		init: function() {
			...
			/*//拖动
			window.addEventListener("touchmove", function(e) { _this.move(e);});
			window.addEventListener("mousemove", function(e) { _this.move(e);});
			//鼠标松开
			window.addEventListener("touchend", function() {_this.end();});
			window.addEventListener("mouseup", function() {_this.end();});*/
			
			addEvent(_this.$element, "touchmove", function(e) {
				_this.move(e)
			});
			addEvent(_this.$element, "mousemove", function(e) {
				_this.move(e)
			});
			addEvent(_this.$element, "touchend", function() {
				_this.end()
			});
			addEvent(_this.$element, "mouseup", function() {
				_this.end()
			});
			
			//鼠标hover时显示验证码图片
			_this.$element.find(".verify-move-block").hover(function() {
				_this.$element.find('.verify-img-panel').css('display', 'block');
				_this.$element.find('.verify-sub-block').css('display', 'block');
			}, function() {
				_this.$element.find('.verify-img-panel').css('display', 'none');
				_this.$element.find('.verify-sub-block').css('display', 'none');
			});
		...
		loadDom: function() {
			...
			panelHtml += '<div class="verify-bar-area"><span  class="verify-msg">滑动完成拼图</span><div class="verify-left-bar"><span  class="verify-msg"></span><div  class="verify-move-block"><i  class="verify-icon iconfont icon-right"></i>' + tmpHtml + '</div></div></div>';
		...
		start: function(e) {
			...
			//于莲莲修改
			this.htmlDoms.move_block.css({'background-color':'#337ab7','border':'1px solid #337ab7'})
			...
			if(this.status) {
				...
				if(this.options.type != 1) { //图片滑动
					...
					if(parseInt(this.htmlDoms.gap.css('left')) >= (parseInt(this.htmlDoms.move_block.css('left')) - vOffset) && parseInt(this.htmlDoms.gap.css('left')) <= (parseInt(this.htmlDoms.move_block.css('left')) + vOffset)) {
						//于莲莲修改
						this.htmlDoms.move_block.css({'background-color':'#5cb85c','border':'1px solid #5cb85c'});
						this.htmlDoms.left_bar.css({
							'border-color': '#5cb85c',
						...
						//于莲莲修改
						this.htmlDoms.move_block.css({'background-color':'#d9534f','border':'1px solid #d9534f'});
						...
						setTimeout(function() {
                            ...
                            //于莲莲修改
							_this.htmlDoms.move_block.css({'background-color':'#fff','border':'0px solid #ddd'});
							_this.htmlDoms.icon.css('color', '#000');
							_this.htmlDoms.icon.removeClass('icon-close');
							_this.htmlDoms.icon.addClass('icon-right');
							_this.$element.find('.verify-msg:eq(0)').text('滑动完成拼图');
